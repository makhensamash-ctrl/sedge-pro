...
const CRM = () => {
  ...
  const handleExportLeads = () => {
    ...
    const formatLead = (l: Lead) => {
      const assignees = (assignmentsMap.get(l.id) || [])
        .map((id) => adminMap.get(id) || id)
        .join(", ");
      const stage = stages.find((s) => s.id === l.stage_id)?.name || "";
      const priceCents = l.package ? packagePriceMap.get(l.package) || 0 : 0;
      return {
        ...
        Source: l.source || "",
        Package: l.package || "",
        "Package Value (ZAR)": priceCents ? priceCents / 100 : "",
        Stage: stage,
        Assignees: assignees,
        "Generated By": l.generated_by || "",
        Notes: l.notes || "",
    ...
    // All Leads sheet
    const allRows = leads.map(formatLead);
    const allSheet = XLSX.utils.json_to_sheet(allRows);
    XLSX.utils.book_append_sheet(wb, allSheet, "All Leads");

    // One sheet per stage
    sortedStages.forEach((stage) => {
      const stageLeads = leads.filter((l) => l.stage_id === stage.id);
      if (stageLeads.length === 0) return;
      const sheet = XLSX.utils.json_to_sheet(stageLeads.map(formatLead));
      const safeName = stage.name.replace(/[\\/*?:[\]]/g, "").slice(0, 31) || "Stage";
      XLSX.utils.book_append_sheet(wb, sheet, safeName);
    });

    const date = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `crm-leads-${date}.xlsx`);
