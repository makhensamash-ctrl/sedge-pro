...
const Quotations = () => {
  ...
  const { data: quotations = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('quotations').select(`*, clients(name, email, phone, address, company), business_profiles(business_name, business_logo, contact_phone, website_address, physical_address, bank_name, account_holder_name, account_number, branch_code, terms_and_conditions)`).order('created_at', { ascending: false });
